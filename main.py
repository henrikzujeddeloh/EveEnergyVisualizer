...
def show_date(data_frame):
    ...
    data_frame['datetime'] = data_frame['datetime'].dt.date
...
def show_heatmap(data_frame):
    ...
    hour_weekday = hour_weekday.astype(float)
...
def show_month(data_frame):
    ...
    # add total energy per month by accessory
    data_frame = data_frame.groupby([data_frame.datetime.dt.to_period("M"), 'accessory'])['energy'].sum().unstack()

    print(data_frame)

    fig_month, axs_month = plt.subplots(figsize=[WIDTH, HEIGHT])
    data_frame.plot.bar(ax=axs_month, rot=0)
...
parser.add_argument("--month", help="output energy consumption by month", action="store_true")
